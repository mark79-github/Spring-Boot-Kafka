...
public class KafkaMessageModelDeserializer extends StdDeserializer<KafkaMessageModel> {
  ...
  public KafkaMessageModel deserialize(
    ...
    EventType eventType =
        getJsonEventType(jsonParser, node, KafkaMessageJsonFieldConstants.EVENT_TYPE);
    TaskModelType taskModelType =
        getJsonTaskModelType(jsonParser, node, KafkaMessageJsonFieldConstants.TASK_MODEL_TYPE);
    ...
    String processorDisplayName =
        getJsonStringValue(node, KafkaMessageJsonFieldConstants.PROCESSOR_DISPLAY_NAME);
    String processorEmail =
        getJsonStringValue(node, KafkaMessageJsonFieldConstants.PROCESSOR_EMAIL);
    ArrayList<RecipientModel> recipientsEmail =
        getJsonRecipientsEmail(node, KafkaMessageJsonFieldConstants.RECIPIENTS_EMAIL);

    return new KafkaMessageModel(
        task,
        eventType,
        taskModelType,
        tenantId,
  ...
  private String getJsonStringValue(JsonNode node, String field) {
    ...
    return jsonNode.isNull() || !jsonNode.isTextual() ? null : jsonNode.asText();
  }

  private TaskModelType getJsonTaskModelType(JsonParser jsonParser, JsonNode node, String field)
      throws JsonMappingException {
    JsonNode taskModelType = node.get(field);
    if (taskModelType == null
        || !EnumUtils.isValidEnum(TaskModelType.class, taskModelType.asText())) {
  ...
  private EventType getJsonEventType(JsonParser jsonParser, JsonNode node, String field)
      throws JsonMappingException {
